...
class Vhm_Share_Buttons_Public {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'vhm-share-buttons-fontawesome', 'https://use.fontawesome.com/releases/v5.0.13/js/all.js', false, $this->version, true );
	}
	...
	public function the_content($post_content)
	{
		...
		if (is_array($source) && is_singular($source) && $show_in_page == 'on') 
		{
			...
			if (in_array("google", $applications)) {
				...
				if ($icons == 'default') { $output .= '<i class="fab fa-google-plus-g"></i> ' . __('Google+', $this->plugin_name); }
				elseif ($icons == 'icon') { $output .= '<i class="fab fa-google-plus-g"></i>'; }
